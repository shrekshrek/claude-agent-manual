...
| Phase 2 | 逐模块实现 | `/plan` → 执行 → 验证 → `/code-review` → `/commit` | 可运行的代码 + 测试 |
...
> **注意**: `/plan` 的输出只存在于当前会话的对话上下文中，不会自动保存到项目文件。
> 确认计划后，建议让 Claude 把关键步骤写入 CLAUDE.md，这样即使会话中断，下次也能接续：
>
> ```
> 把刚才确认的实施计划要点更新到 CLAUDE.md 的"当前进度"部分
> ```

#### 第二步：执行开发

计划确认后，`Shift+Tab` 切到 Auto-accept 模式，选择执行。Claude 会按计划完成实现并编写测试。

**核心业务逻辑建议用 `/tdd` 替代直接执行**：支付计算、权限判断、数据转换等边界多且出错代价高的功能，用 `/tdd` 能迫使 Claude 先定义接口契约和边界条件，再写实现，质量更有保障。
