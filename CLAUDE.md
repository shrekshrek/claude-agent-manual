...
## 项目简介
Claude Code 插件，提供按模块推进的项目工作流管理。五个命令覆盖项目生命周期：`/project-plan` → `/module-plan` → `/module-dev` → `/module-done`，外加 `/plan-review` 进行方案质量审查。
...
| `commands/` | 命令 | `/project-plan`、`/module-plan`、`/module-dev`、`/module-done`、`/plan-review` |
