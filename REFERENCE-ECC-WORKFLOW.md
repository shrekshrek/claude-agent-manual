...
ECC 新增了 PM2 & Multi-Agent 编排能力，适用于多服务项目的并行开发：

| 命令 | 用途 |
|:---|:---|
| `/pm2` | PM2 进程管理（启动、监控、重启服务） |
| `/multi-plan` | 多 Agent 并行规划（前后端同时出方案） |
| `/multi-execute` | 多 Agent 并行执行（独立模块同时开发） |
| `/multi-backend` | 后端专用多 Agent 编排 |
| `/multi-frontend` | 前端专用多 Agent 编排 |
| `/multi-workflow` | 自定义工作流编排 |

> 这些命令将 Boris Cherny 的"多终端并行"模式从手动操作升级为命令化编排，适合微服务或前后端分离项目。

---

## 3. Rules 体系

ECC 通过 `~/.claude/rules/` 下的规则文件定义 AI 行为规范。规则已重组为按语言分目录，安装时可按需选择：
...
│   ├── hooks.md
│   └── performance.md
├── typescript/          ← TypeScript/React 项目
│   ├── coding-style.md
│   └── patterns.md
├── python/              ← Python 项目
└── golang/              ← Go 项目
...
| iterative-retrieval | 渐进式上下文检索，解决 subagent 上下文不足问题 |
| strategic-compact | 在逻辑间断点建议手动压缩上下文，避免自动压缩丢失关键信息 |

**Instinct 管理命令**（配合 continuous-learning-v2 使用）：

| 命令 | 用途 |
|:---|:---|
| `/instinct-status` | 查看所有已学习的经验及置信度 |
| `/instinct-import` | 从队友或其他项目导入经验 |
| `/instinct-export` | 导出经验分享给他人 |
| `/evolve` | 将相关经验聚合演化为 skill / command / agent |
| `/skill-create` | 分析本地 git 历史，提取编码模式生成 SKILL.md |
...
**替代方案**：将 MCP 功能封装为 Skills/Commands 以节省上下文。例如用 `/gh-pr` 命令包装 `gh pr create`，而不是始终加载 GitHub MCP。

### Token Economics（来自 Longform Guide）

ECC Longform Guide 提供了量化的 token 优化指导：

**模型选择是最大的优化杠杆**：Haiku 与 Opus 的成本差为 **18.75x**。策略性地将辅助任务分配给 Haiku 可以在不牺牲质量的前提下降低一个数量级的成本。

**MCP 数量控制**：
- 启用的 MCP **< 10 个**
- 活跃工具总数 **< 80 个**
- 超出会导致可用上下文从 ~130k 降至 ~70k

**核心原则**：**Configure broadly, enable narrowly, compact proactively**（广泛配置，精准启用，主动压缩）。

**代码库模块化**：保持单文件在**几百行**而非几千行，既降低 token 消耗，也提高一次做对的概率。

### CLAUDE.md 最佳实践：Progressive Disclosure

ECC Longform Guide 推荐 CLAUDE.md 采用 **渐进式披露（Progressive Disclosure）** 模式：

**核心原则**：CLAUDE.md 做路由表，不做百科全书。只放每次会话都需要的信息，其余放单独文件由 Claude 按需读取。

**推荐结构**：

```
CLAUDE.md                    ← 路由表：项目概况 + 文件清单 + 一行描述
agent_docs/                  ← 详细文档目录
├── architecture.md          ← 系统架构
├── current_plan.md          ← 当前实施方案
...
```markdown
## 参考文档
- `agent_docs/architecture.md` — 系统架构与模块边界
- `agent_docs/current_plan.md` — 当前迭代的实施方案
Claude 开始工作前应先判断哪些文件与当前任务相关并阅读。
```

**为什么不把所有内容放 CLAUDE.md？**
- Claude Code 在系统提示中注入了 *"this context may or may not be relevant"*，不相关的内容会被主动忽略
- CLAUDE.md 每次会话自动加载，内容越多，有用指令被稀释的概率越高
- Boris Cherny 的 CLAUDE.md 仅 ~2.5k tokens（约 100 行），社区建议 < 300 行

### 跨会话记忆

ECC Longform Guide 推荐在 `.claude/` 下用 `.tmp` 文件保存会话进度摘要：

```
1. 会话结束前，Claude 生成当前状态摘要并保存到 .claude/session-summary.tmp
2. 下次会话开始时，提供该文件路径作为上下文
3. 每次会话创建新文件，避免旧上下文污染新工作
```

这与 Anthropic 长周期 Agent 方案中的 `claude-progress.txt` 思路一致，但更轻量。
...
**验证策略选择**（来自 Longform Guide）：

| 策略 | 含义 | 适用场景 |
|:---|:---|:---|
| **pass@k** | k 次尝试中至少 1 次成功 | 探索性任务，只要能跑通即可 |
| **pass^k** | k 次尝试必须全部成功 | 支付、权限等一致性要求高的场景 |

此外还支持**持续验证**：每 N 分钟或每次重大变更后自动运行验证，而非仅在结束时检查。
...
| 领域 | ECC 现状 | 补充方案 |
|:---|:---|:---|
| 跨会话持续性 | Longform Guide 有覆盖（`.tmp` 文件方案，见上文） | 也可结合 claude-progress.txt（Anthropic 方案） |
...
- [ECC Longform Guide](https://github.com/affaan-m/everything-claude-code/blob/main/the-longform-guide.md)
- [ECC Shortform Guide](https://github.com/affaan-m/everything-claude-code/blob/main/the-shortform-guide.md)
- [Token Optimization Strategies - DeepWiki](https://deepwiki.com/affaan-m/everything-claude-code/12.2-token-optimization-strategies)
