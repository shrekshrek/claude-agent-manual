...
- **主观模式信号 / hook 自动触发** → v2 不做(违反 [workflow.md §0.5 信念 1](../../docs/workflow.md#05-实现策略的核心信念) "消除对齐劳动" —— 系统主动 nudge 本身就是新对齐源)
...
- **只做 Critical 客观 drift**。主观信号(模式 / 弱信号 / hook 自动触发)v2 不做 —— 违反 [§0.5 信念 1](../../docs/workflow.md#05-实现策略的核心信念)
- **不自动 commit**。任何 patch 应用都留给用户最终决定
- **可中断**(`q`uit):已 apply 的保留,未决的丢弃
- **`.claude/refresh-ignore` 默认进 git**(团队共享 ignored drift 决策);需要私有(个人偏好)时手动加 `.gitignore`
- **跟 `/spec-revise` 互补**:`/spec-revise` 是 feature-driven reactive(in-feature 触发);本 skill 是 phase-driven proactive(P4 周期 / 感知 drift)
- **跟 `/proof-bundle` Item 5b 互补**:Item 5b 是 per-feature backlog(本 feature 派生未应用);本 skill 是 project-wide audit(全项目周期扫描)
